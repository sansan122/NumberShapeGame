...
#
# ⚠️ 但**不能挑写在 `#` 注释里的句子**：注释不进字节码，
#    指纹落在注释上必然 FAIL。这次加「查找遗物」的指纹时就又踩了一遍
#    （「哪些节点会出遗物」「先问左侧面板上的按钮」两句都写在注释里，
#     本地预检与打包自检一起报落空）。挨着它们的**字符串字面量**才稳：
#     docstring、报错文案、tooltip 文案、日志文案。
#    —— 一句话：**指纹要从「会进字节码的文字」里挑。**
#
...
FINGERPRINTS = [
    ...
    ("map_scene", "地图能查找遗物来源", "遗物来源 · %s：%s"),
    ("map_scene", "侧栏布局只有一处", "左侧状态面板的**唯一**布局来源"),
    ("map_scene", "遗物图鉴面板", "遗物图鉴：整池一次列全"),
    ("map_scene", "图鉴卡片高度按行数算", "永远留在屏幕里"),
    ("player", "十件遗物取名即口诀", "名字本身就是这条效果的口诀"),
    ("player", "约分在删牌定价里打折", "遗物「约分」在这里打折"),
    ("battle_scene", "等比数列按答对题数加成", "「等比数列」生效：本回合答对 %d 题"),
    ("battle_scene", "等周不等式要数字卡垫场", "「等周不等式」生效：+%d 格挡"),
    ("battle_scene", "反例被命中时反弹", "「反例」生效：反弹 %d 点伤害"),
    ("battle_scene", "守恒律开场回血", "「守恒律」生效：恢复 %d 点生命"),
    ("battle_scene", "胜负结算只发一次钱", "钱和血必须写在「只结算一次」的门里面。"),
    ("map_scene", "查找高亮的横幅", "本层 %d 处：%s"),
    ("map_scene", "未完待证不许剧透", "未完待证：这一格要走到才看得清"),
    ("main", "侧栏按钮优先判于节点", "查找遗物：开"),
    ("main", "玩家面板只剩一份实现", "实现在 map_scene.MapScene.draw_side"),
    ("game_env", "折行逻辑收口到一处", "把 text 按像素宽度折成若干行"),
    ("deck_view", "牌组折行改用共用实现", "实现已收口到 game_env.wrap_text"),
    ("art_shapes", "遗物也有图案", "按遗物名在 center 画一个 size"),
    ("art_shapes", "遗物图案：等比数列", "1 : 2 : 4 三块，底边对齐"),
    ("art_shapes", "遗物图案：反例弹回", "一支箭头撞上竖墙、原路弹回去"),
    ("art_shapes", "遗物图案：定义域扩张", "四个斜角还探出小箭头"),
    ("art_shapes", "遗物图案漏配会画问号", "这件遗物配了专属图案吗"),
    ("art_shapes", "箭头统一在这里画", "在 tip 处画一个指向 ang（弧度）的实心箭头"),
    ("map_scene", "图鉴卡片内元素怎么摆", "一张卡片里的元素怎么摆"),
    ("map_scene", "按钮小图案的坐标也只有一处", "按钮左边那个小图案的位置"),
    ("map_scene", "没到手的图案变灰但还看得清", "没到手的那张，图案也跟着变灰"),
    ("game_env", "折行按不可拆的块", "把一句话切成「不可拆的块」"),
    ("game_env", "断行优先断在标点后", "中文没有空格，逐字折行会把「伤害」"),
    ("game_env", "折完再对齐一次不再多出行", "把折好的行再对齐一次"),
    ("node_scenes", "宝箱结果卡带图案", "未打开时画箱子；打开后画结果卡"),
    ("node_scenes", "战利品遗物页带图案", "战利品第一页：遗物（图案 + 名字 + 效果文案）"),
]
